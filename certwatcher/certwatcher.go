...
import (
	...
	"k8s.io/utils/certwatcher/metrics"
	logf "k8s.io/utils/log"
)

var log = logf.Log.WithName("certwatcher")
...
func (cw *CertWatcher) Start(ctx context.Context) error {
...

	{
		...
		if !addWatches() {
			...
			for {
				select {
				case <-ctx.Done():
					return fmt.Errorf("failed to add watches: %w", errors.Join(ctx.Err(), watchErr))
				case <-deadline:
					return fmt.Errorf("failed to add watches: %w", errors.Join(fmt.Errorf("timed out waiting for watches"), watchErr))
				case <-ticker.C:
					if addWatches() {
						break loop
					}
	...
	pollTicker := time.NewTicker(cw.interval)
